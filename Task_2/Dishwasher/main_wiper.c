...
int main()
{
	...
	strcpy(info.msgto.pathname, "ForSecondKey.txt");
	...
	strcpy(info.msgfrom.pathname, "ForFirstKey.txt");
	...
	GetTime(info.dishes, info.time, towipe);
	...
	for (info.i = 0; info.i < 4; (info.i)++)
	{
		...
		result = pthread_create(&thid_first, (pthread_attr_t*)NULL, WaitForMessage, &info);
		...
		result = pthread_create(&thid_second, (pthread_attr_t*)NULL, Wipe, &info);
